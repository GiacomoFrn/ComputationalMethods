...
//POISSON EQUATION SOLVER FOR plate capacitor (2D)
...
int main(){
	...
	ofstream file("p_r.txt");    // charge distribution output file
	ofstream file1("phi_r.txt"); // potential output file
	...
	for(int l = 0; l< nx*ny; ++l  ){
		...
		if((i<35 )&& (i>30)&&(j>10)&&(j<90)) p.at(l) = p0;
		else if((i<80 )&& (i>75)&&(j>10)&&(j<90)) p.at(l) = -p0;
	...
	do{
		...
		//double alpha = 0.002;
		for(int m = 0; m< nx*ny; ++m ){ // GAUSS SIEDEL
			i=geti(m);
			j=getj(m);
			phi_1.at(m) = -f1*phi_0.at(getl(i+1,j))/getMlm(m,m);
			phi_1.at(m) += -f1*phi_1.at(getl(i-1,j))/getMlm(m,m);
			phi_1.at(m) += -f1*phi_0.at(getl(i,j+1))/getMlm(m,m);
			phi_1.at(m) += -f1*phi_1.at(getl(i,j-1))/getMlm(m,m);
			phi_1.at(m) += -p.at(m)/getMlm(m,m);
	    }
		/*for(int m = 0; m< nx*ny; ++m ){ // RICHARDSON
			i=geti(m);
			j=getj(m);
			phi_1.at(m) = phi_0.at(m);
			phi_1.at(m) += alpha*f1*phi_0.at(getl(i+1,j))/getMlm(m,m);
			phi_1.at(m) += alpha*f1*phi_0.at(getl(i-1,j))/getMlm(m,m);
			phi_1.at(m) += alpha*f1*phi_0.at(getl(i,j+1))/getMlm(m,m);
			phi_1.at(m) += alpha*f1*phi_0.at(getl(i,j-1))/getMlm(m,m);
			phi_1.at(m) += -alpha*p.at(m)/getMlm(m,m);
	    }*/
